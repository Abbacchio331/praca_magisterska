...
class YouTubeSession:
    ...
    async def find_and_a_play_song(self, title: str, wait_time: int = 30):
        if self.page is None:
            raise RuntimeError("Strona się nie otworzyła.")
        ...
        if 'aria-controls="suggestion-list"' in page_html:
            ...
            return False, "Nie znaleziono pola wpisywania."
        ...
        if not page_type_search or not findall(r'(?i)\b(play|songs|undercards)\b', page_type_search[0]):
            print("Przycisk odtwarzania nie został odnaleziony.")
            return False, "Przycisk odtwarzania nie został odnaleziony."
        ...
        if findall(r'(?i)\b(play)\b', page_type_search[0]):
            ...
            title_search: list = findall(r'class="title style-scope ytmusic-card-shelf-renderer"(?:[^>]*?>){2}\s*([^<]*?)\s*<', found_page_html, DOTALL)
            ...
            title_search: list = findall(r'(?i)ytmusic-shelf-renderer"[^>]*?>\s*songs\s*<.*?title="\s*([^"]*?)\s*"', found_page_html, DOTALL)
